const { assert, expect } = require("chai");
const { developmentChains } = require("../../helper-hardhat-config");

const highImageURI = "data:image/svg+xml;base64,";
const lowImageURI = "data:image/svg+xml;base64,";

const highTokenUri =
    "data:application/json;base64,"
const lowTokenUri =
    "data:application/json;base64,"

!developmentChains.includes(network.name) ?
    describe.skip :
    describe("DynamicSvgNft", () => {
        let deployer, dynamciSvgNft;
        beforeEach(async () => {
            deployer = (await getNamedAccounts()).deployer
            await deployments.fixture(["mocks", "dynamicSvg"]);
            dynamciSvgNft = await ethers.getContract("DynamicSvgNft");
        })

        describe("constructor", () => {
            it("it intializes the Dynamic Nft correctly", async () => {
                const tokenCounter = await dynamciSvgNft.getTokenCounter();
                const lowSvg = await dynamciSvgNft.getLowSVG();
                const highSvg = await dynamciSvgNft.getHighSVG();
                assert.equal(tokenCounter, 0);
                assert.equal(lowImageURI, lowSvg);
                assert.equal(highImageURI, highSvg);
            })
        })

        describe("mintNFt", () => {
            let mintNft;
            beforeEach(async () => {
                const highValue = ethers.utils.parseEther("1");
                mintNft = await dynamciSvgNft.mintNft(highValue);
            })

            it("it mints the Nft , increase the tokenCounter", async () => {
                const tokenCounter = await dynamciSvgNft.getTokenCounter();
                assert.equal(tokenCounter, 1);
            })

            it("emits the event on enter", async () => {
                await expect(mintNft).to.emit(dynamciSvgNft, "CreatedNFT");
            })

            it("records the highvalue corresponds to the token counter", async () => {
                const tokenCounter = await dynamciSvgNft.getTokenCounter();
                const highValue = await dynamciSvgNft.getHighValue(tokenCounter - 1);
                assert.equal(highValue.toString(), "1000000000000000000");
            })
        })
        describe("calls the tokenuri", () => {
            it("reverts with error", async () => {
                const highValue = ethers.utils.parseEther("10000000");
                const txResponse = await dynamciSvgNft.mintNft(highValue);
                await txResponse.wait(1);
                await expect(dynamciSvgNft.tokenURI(1)).to.be.revertedWith("ERC721Metadata__URI_QueryFor_NonExistentToken");
            })

            it("shifts the tokenuri to lower value when the price doesn't surpass the highvalue", async () => {
                const highValue = ethers.utils.parseEther("10000000");
                const txResponse = await dynamciSvgNft.mintNft(highValue);
                await txResponse.wait(1);
                const tokenUri = await dynamciSvgNft.tokenURI(0);
                assert.equal(tokenUri, lowTokenUri);

            })
        })
    })